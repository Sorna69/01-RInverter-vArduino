...
void confGPIOPCNT()
{
    ...
    // Set the GPIO_FUNCy_IN_SEL field in this register, corresponding to the GPIO pad X to read from. Clear
    // all other fields corresponding to other GPIO pads.
    REG_SET_FIELD(GPIO_FUNC39_IN_SEL_CFG_REG, GPIO_FUNC39_IN_SEL, 36);
    //REG_SET_FIELD(GPIO_FUNC39_IN_SEL_CFG_REG, GPIO_FUNC39_IN_SEL, 16);

    REG_SET_FIELD(GPIO_FUNC41_IN_SEL_CFG_REG, GPIO_FUNC41_IN_SEL, 0x30);

    // REG_SET_FIELD(GPIO_FUNC43_IN_SEL_CFG_REG, GPIO_FUNC43_IN_SEL, 39);
    // REG_SET_FIELD(GPIO_FUNC47_IN_SEL_CFG_REG, GPIO_FUNC47_IN_SEL, 34);
    ...
    REG_SET_FIELD(GPIO_FUNC16_OUT_SEL_CFG_REG, GPIO_FUNC16_OEN_SEL, 1);

    // 2b)
    // The GPIO_ENABLE_DATA[x] field is a bit in either GPIO_ENABLE_REG (GPIOs 0-31) or GPIO_ENABLE1_REG
    //(GPIOs 32-39). Clear this bit to disable the output driver for the GPIO pad.
    // REG_SET_FIELD(GPIO_ENABLE_REG, GPIO_ENABLE_DATA, BIT15 + BIT18);
    REG_SET_BIT(GPIO_ENABLE1_W1TC_REG, BIT4);
    REG_SET_BIT(GPIO_ENABLE_W1TC_REG, BIT16);

    // REG_CLR_BIT(GPIO_ENABLE1_REG, BIT2 + BIT4 + BIT7);
    ...
    REG_SET_FIELD(IO_MUX_GPIO16_REG, FUN_IE, 1);
    ...
    REG_SET_BIT(IO_MUX_GPIO16_REG, BIT8);
}
...
void confPCOUNTER()
{
    ...
    REG_SET_FIELD(PCNT_U0_CONF2_REG, PCNT_CNT_H_LIM_U0, 15);
    REG_SET_FIELD(PCNT_U0_CONF0_REG, PCNT_THR_THRES0_EN_U0, 0);
    REG_SET_FIELD(PCNT_U0_CONF0_REG, PCNT_THR_L_LIM_EN_U0, 0);
}

void startPCOUNTER()
{
    // REG_CLR_BIT(PCNT_CTRL_REG);
    REG_SET_FIELD(PCNT_CTRL_REG, PCNT_PLUS_CNT_RST_U0, 0);
}

void ResetStopPCOUNTER()
{
