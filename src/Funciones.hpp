...
void onWsEvent(AsyncWebSocket *server, AsyncWebSocketClient *client, AwsEventType type, void *arg, uint8_t *data, size_t len)
{
  if (type == WS_EVT_CONNECT)
  {
    ...
    delay(500);
    initialStatus(DIS, digitalRead(DIS), client->id());
    initialStatus(PWM_A, digitalRead(PWM_A), client->id());

    //updateGPIO(PWM_A, digitalRead(PWM_A), client->id());
    //updateGPIO(PWM_B, digitalRead(PWM_B), client->id());
  }
